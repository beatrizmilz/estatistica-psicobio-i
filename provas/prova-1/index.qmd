...
-   O que 112% de confiança significa? É possível ter IC maior que 100%?
...
As três representações permite obter informações da distribuição dessa variável (chamada aqui de conjunto de dados).
...
B - O histograma possibilita visualizar a distribuição da variável. Conseguimos ver que a maior frequência dos valores estão entre 15 e 20.

C - O dotplot permite ver cada um dos valores, e permite ver a concentração de dados entre os valores 10 e 20.

::: callout
Pensar em mais pontos!
:::
...
-   [ ] Amostras com tamanhos desiguais podem gerar vieses nos resultados do teste t.

-   [ ] Todas estão corretas

::: callout
Para mim não ficou claro a diferença entre as duas linhas da tabela (equal/unequal).
:::
...
Podemos usar o teste Wilcoxon, um teste não paramétrico.

Os resultados do teste de wilcoxon (W = 2,22) apontam que não existe diferença entre os grupos (escola pública e escola privada) na média de palavras lembradas antes do treino de motrocidade.
...
::: callout
Dado que estamos comparando, não seria melhor usar o mesmo teste nessas duas questões? (11 e 12)
:::
...
-   [ ] Gráficos de barra são mais adequados para comparar valores individuais entre diferentes grupos, enquanto boxplots são mais úteis para visualizar a distribuição dos dados dentro de um único grupo.
...
-   [ ] Se o intervalo de confiança para a diferença entre os momentos incluir o valor zero, podemos concluir que não há diferença significativa entre os momentos.
...
Precisamos fazer testes qui-quadrado!
